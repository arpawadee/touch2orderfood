<?php
	class functions {
		...
		function sanitize($string) {
			// check string value
			$link = mysqli_connect("localhost", "id9808710_arpawadee", "m044088039", "id9808710_t2orderfood");

			$string = mysqli_real_escape_string($link,trim(strip_tags(stripslashes($string))));
			return $string;
		}// end of sanitize()
		...
		function doPages($page_size, $thepage, $query_string, $total=0, $keyword) {
			...
			echo '<div id="page_num" class="body right">';
			...
			if ($total != 0){
				echo '<br><div class="right"><h5>( total '.$total.' )</h5></div></div>';
			}else {
				echo '</div>';
			};
		}
